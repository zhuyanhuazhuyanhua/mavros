...
public:
  mavros_ctrl(ros::NodeHandle &nh)
      : tf_buffer_(), tf_listener_(tf_buffer_, nh), static_tf_broadcaster_(), is_grounded_(false)
  {
    // 初始化发布器和订阅器
  ...
  void spin()
  {
    ...
    {
      ROS_INFO("Vehicle is grounded, waiting for takeoff");
      return;
    }
...
private:
  ...
  void target_pos_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
  {
    ...
    // 判断 z 坐标是否小于 -1
    if (target_positions.pose.position.z < -0.9)
    {
      // 尝试上锁
      mavros_msgs::CommandBool arm_cmd;
      arm_cmd.request.value = false; // 设置为 false 表示上锁
      ...
      if (arming_client_.call(arm_cmd))
      {
        ...
        {
          is_grounded_ = true;
          ROS_INFO("Vehicle disarmed due to low z position");
        }
        else
        {
          ROS_ERROR("Disarming failed: %s",
                    arm_cmd.response.success ? "true" : "false");
        }
      }
      else
